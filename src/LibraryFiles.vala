...
namespace LibraryFiles {
    ...
    time_t? get_timestamp_from_metadata(MediaMetadata? metadata) {
        if (metadata == null) {
            return null;
        }
    ...
    time_t? get_timestamp_from_file_modified(File src) {
        try {
            return query_file_modified(src);
        } catch (Error err) {
    ...
    time_t get_timestamp_from_file_modified_and_metadata(File src, MediaMetadata? metadata) {
        time_t? timestamp = get_timestamp_from_metadata(metadata);
        if (timestamp != null)
            return timestamp;
        timestamp = get_timestamp_from_file_modified(src);
        if (timestamp != null)
            return timestamp;
        return time_t();
    }
    ...
    public File? generate_unique_file(string basename, MediaMetadata? metadata, time_t ts, out bool collision) throws Error {
        stderr.printf("doing generate unique file for %s...\n", basename);
    ...
    public File? generate_unique_file_fast(string basename, out bool collision) throws Error {
        // build a directory tree inside the library
        File staging_dir = AppDirs.get_staging_import_dir();

        // Optionally convert to lower-case.
        string newbasename = convert_basename(basename);

        return global::generate_unique_file(staging_dir, newbasename, out collision);
    }
    ...
    public File move_to_datedir(File in_staging_dir, MediaMetadata? metadata) {
        time_t ts = get_timestamp_from_file_modified_and_metadata(in_staging_dir, metadata);
        // move it from in_staging_dir to baked_import_dir
        bool _ignored;
        File dest = generate_unique_file(in_staging_dir.get_basename(), metadata, ts, out _ignored);
        in_staging_dir.move(dest, FileCopyFlags.OVERWRITE | FileCopyFlags.ALL_METADATA);
        return dest;
    }
    ...
    private File duplicate(File src, FileProgressCallback? progress_callback, bool blacklist) throws Error {
        time_t timestamp = get_timestamp_from_file_modified(src);
