...
public class BatchImport : Object {
    ...
    private Gee.List<CompletedImportObject> ready_thumbnails = new Gee.LinkedList<CompletedImportObject>();
    private Gee.List<CompletedImportObject> display_imported_queue = new Gee.LinkedList<CompletedImportObject>();
    ...
    // Called repeatedly to report the progress of the BatchImport (but only called after the last "preparing" signal)
    public signal void progress(uint64 completed_bytes, uint64 total_bytes);

    // Called for each Photo or Video imported to the system. For photos, the pixbuf is screen-sized and rotated. For videos, the pixbuf is a frame-grab of the first frame.
    //
    // The to_follow number is the number of queued-up sources to expect following this signal in one burst.
    public signal void imported(MediaSource source, Gdk.Pixbuf pixbuf, int to_follow);

    // Called when a fatal error occurs that stops the import entirely.  Remaining jobs will be failed and import_complete() is still fired.
        ...
        if (import_result.result != ImportResult.SUCCESS) {
            ...
            if (import_result.file != null && !import_result.result.is_abort()) {
                ...
                try {
                    ...
                    warning("Unable to query file size of %s: %s", import_result.file.get_path(), err.message);
                }
    ...
    private void file_import_complete() {
        // This should be called whenever a file's import process is complete,
        ...
        if (file_imports_to_perform != -1) {
            ...
            // last file
            if (file_imports_completed == file_imports_to_perform) {
    ...
    private void flush_import_jobs() {
        ...
        // has been written)
        while (
            ready_thumbnails.size > 0 &&
            import_workers.get_pending_job_count() < max_outstanding_import_jobs
        ) {
            import_workers.enqueue(
                new ThumbnailWriterJob(
                    this,
                    ready_thumbnails.remove_at(0),
                    on_thumbnail_writer_completed,
                    cancellable,
                    on_thumbnail_writer_cancelled
                )
            );
        }
        while(
            ...
            import_workers.enqueue(
                new PreparedFileImportJob(
                    this,
                    ready_files.remove_at(0),
                    import_roll.import_id,
    ...
    private async void process_prepared_files(Gee.List<PreparedFile> list) {
        foreach (PreparedFile prepared_file in list) {
            ...
            LibraryPhoto? photo = LibraryPhoto.global.get_state_by_file(prepared_file.file, out photo_state);
            if (photo != null) {
                switch (photo_state) {
                    ...
                    case LibraryPhotoSourceCollection.State.DEVELOPER:
                        import_result = new BatchImportResult(
                            prepared_file.job, prepared_file.file,
    ...
    private void on_import_files_completed(BackgroundJob j) {
        ...
        if (job.ready != null) {
            ...
            if (job.ready.batch_result.result == ImportResult.SUCCESS) {
                ready_thumbnails.add(
                    new CompletedImportObject(
                        source,
                        job.ready.get_thumbnails(),
                        job.ready.prepared_file.job,
    ...
    // Because the LibraryPhoto has been created at this stage, any cancelled work must also destroy the LibraryPhoto.
    ...
    private void flush_ready_sources() {
        ...
        foreach (MediaSource media in completion_list.keys) {
            try {
                ...
                warning("Completion error when finalizing import of %s: %s", media.to_string(), err.message);
            }
        }
    ...
    private bool display_imported_timer() {
        ...
        if (!cancellable.is_cancelled()) {
            if (display_imported_queue.size > DISPLAY_QUEUE_HYSTERESIS_OVERFLOW)
                total = 1 << ((display_imported_queue.size / DISPLAY_QUEUE_HYSTERESIS_OVERFLOW) + 2).clamp(0, 16);
        } else {
...
#if TRACE_IMPORT
        if (total > 1) {
            debug("DISPLAY IMPORT QUEUE: hysteresis, dumping %d/%d media sources", total, display_imported_queue.size);
        }
#endif
