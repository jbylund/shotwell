...
class AppDirs {
    ...
    public static File get_baked_import_dir(time_t tm) throws Error {
        ...
        File baked_dir = File.new_for_path(get_import_dir().get_path() + Path.DIR_SEPARATOR_S + date.format(pattern));

        return mkdir_p(baked_dir);
    }

    public static File mkdir_p(File to_make) throws Error {
        // build a directory tree
        try {
            to_make.make_directory_with_parents(null);
        } catch (Error err) {
    ...
    // TODO: incorporate the import roll in here somehow?
    public static File get_staging_import_dir() throws Error {
        File dir = File.new_for_path(get_import_dir().get_path() + Path.DIR_SEPARATOR_S + ".staging");
        return mkdir_p(dir);
    }
